package com.adpanshi.cashloan.manage.core.common.util.excel;
...
public class JsGridReportBase {
	...
	public void exportExcel(List<Object> list,String title,String[] hearders,String[] fields,String operatorName) throws Exception{
		...
		if (list.size()> ExportConstant.END_LIMIT) {
			int excelSize = ExportConstant.END_LIMIT;	//每个Excel文件条数
			int totalCount = list.size();	//查询结果总条数
			int pageCount=totalCount%excelSize > 0?totalCount/excelSize + 1:totalCount/excelSize;//页数
			exportToExcel(excelTitle, operatorName, list,pageCount,hearders,fields);
		}else{
	 ...
	 * @throws Exception
	 */
	public void exportModelExcel(String title,String[] hearders) throws Exception{
		String excelTitle = title+"_"+DateUtil.dateStr3(DateUtil.getNow());
		TableData td = new TableData(ExcelUtil.createTableHeader(hearders));
		exportToModelExcel(excelTitle,hearders, td);
	}
	...
	public void exportToExcel(String title, String creator, TableData tableData) throws Exception {
		...
		HashMap<String, HSSFCellStyle> styles = new HashMap<String, HSSFCellStyle>();
	 ...
	 * @param title 文件名
	 * @param creator 创建人
	 * @param list 各工作格数据(注意：每个tableData要设置sheet名称，否则按默认呈现)
	 * @return void <style name="dataset"> case SYSROWNUM%2==0?#row0:#row1; fontsize:9px; </style> <style name="row0">
	 *         import(parent); bgcolor:#FFFFFF; </style> <style name="row1"> import(parent); bgcolor:#CAEAFE; </style>
	 * @throws IOException 
	 */
	public void exportToExcel(String title, String creator, List<Object> list,int pageCount,String[] hearders,String[] fields) throws IOException{

		HSSFWorkbook wb = new HSSFWorkbook();// 创建新的Excel 工作簿
		//HashMap<String, HSSFCellStyle> styles = initStyles(wb);// 初始化表头样
		HashMap<String, HSSFCellStyle> styles = new HashMap<String, HSSFCellStyle>();
		TableData td=null;
		for (int i = 1; i <= pageCount; i++) {
			if (i == pageCount) {
				td = ExcelUtil.createTableData(list.subList((i-1)*ExportConstant.END_LIMIT,list.size()), ExcelUtil.createTableHeader(hearders), fields);
			} else{
				td = ExcelUtil.createTableData(list.subList((i-1)*ExportConstant.END_LIMIT,ExportConstant.END_LIMIT*(i)), ExcelUtil.createTableHeader(hearders), fields);
			}
			td.setSheetTitle(title + "_("+(i)+")");
//			String sheetTitle = td.getSheetTitle();
//			sheetTitle = sheetTitle == null || "".equals(sheetTitle) ? "sheet" + i : sheetTitle;
//			td.setSheetTitle(sheetTitle);
			wb = writeSheet(wb, td.getSheetTitle(), styles, creator, td);// 写入工作表
		}
	 ...
	 *         import(parent); bgcolor:#FFFFFF; </style> <style name="row1"> import(parent); bgcolor:#CAEAFE; </style>
	 */
	public void exportToModelExcel(String title,String [] hearders,TableData tableData) throws Exception {
		XSSFWorkbook wb = new XSSFWorkbook();// 创建新的Excel 工作簿
		XSSFSheet sheet = wb.createSheet(title);
		XSSFRow row = sheet.createRow(0);// 创建新行
		for(int i=0;i<hearders.length;i++){
			XSSFCell cell = row.createCell(i);// 创建新列
			cell.setCellValue(new XSSFRichTextString(hearders[i]));
